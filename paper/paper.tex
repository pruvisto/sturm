...
%Lemma: Let $P\in\RR[X]\setminus\{0\}$. Let $P_0,\ldots,P_{n-1}$ be the result of the canonical Sturm chain construction of $P$ and define $Q_i := P_i / \gcd(P,P')$. Then $Q_0,\ldots,Q_{n-1}$ is a Sturm chain and $Q_0$ has the same roots, disregarding multiplicity, as $P_0$.\\
%Proof:
%\begin{itemize}
%\item since $Q_0,\ldots,Q_{n-1}$ has the same length as $P_0,\ldots,P_{n-1}$, which is the result of the canonical Sturm chain construction, the length of $Q_0,\ldots,Q_{n-1}$ is $\geq 2$.
